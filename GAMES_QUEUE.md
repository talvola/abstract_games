...
| ~~wa_shogi~~ | 11×11 27-piece animal shogi — build agent hit session limit after only game.py; partial REMOVED, DEFERRED to a fresh build | **deferred** |

**Next-wave queue remaining (dedup-verified free):** wa_shogi (retry — fresh full build), marseillais_chess, progressive_chess, monster_chess, omweso, picaria, latrunculi, nard, jul_gonu, yonin_shogi. NOTE: progressive/marseillais/monster chess need multi-move-per-turn handling (verify the engine supports it before assigning); latrunculi/picaria have custom-render or clone-adjacency risk (dedup + browser-verify); yonin_shogi is 4-player (>2-seat render risk). AVOID clone-adjacent hunt/race variants.
