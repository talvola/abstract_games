"""Brazilian Draughts — International (Polish) draughts rules on an 8x8 board.

This is *exactly* the International Draughts ruleset (men capture forward AND
backward, flying kings, mandatory MAXIMUM capture / the "majority" rule,
captured pieces removed at end of move, end-of-move-only promotion) played on
the smaller 8x8 checkers board with 12 men per side. It is the closest possible
sibling of the shipped International Draughts package — only the board size and
starting setup differ.

Men move one square diagonally FORWARD to an empty square, but CAPTURE both
forward and backward. Kings are FLYING: they slide any distance along a diagonal
over empty squares, and capture by leaping a single enemy piece (with any number
of empty squares before it) and landing on any empty square beyond, then
optionally continuing the chain.

Captures are MANDATORY and you must play a sequence that captures the MAXIMUM
number of pieces; among maximal sequences any may be chosen. Captured pieces are
removed only at the END of the move, and a piece may not be jumped twice in one
sequence. A man promotes to king only if it ENDS its move on the last rank.
...
visits, e.g. "1,2>2,3" (simple) or "3,4>5,2>3,0" (a man double-jump). Because
...
class BrazilianDraughts(Game):
    uid = "brazilian_draughts"
