...
_(none — but FYI: the existing committed `pente` lacks a selftest and defaults to FREE opening rather than the standard centre-opening. A deliberate future upgrade could add the authentic `opening` option + a selftest, but must KEEP the hard ply-cap since Pente can loop via captures. Not blocking anything.)_
