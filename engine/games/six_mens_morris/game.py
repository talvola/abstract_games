"""Six Men's Morris -- the smaller mill game on a 16-point board.

A two-phase game: first each player *places* six men one at a time, then they
*slide* men along the board lines. Forming a "mill" (three of your men in a line)
lets you remove one of the opponent's men. Reduce the opponent to two men, or
leave them with no legal move, to win.

The board is two concentric squares with the four mid-side spokes connecting the
outer and inner squares. Unlike Nine Men's Morris there is no middle ring, no
centre cross, and -- crucially -- **no flying phase** and **no mills along the
spokes** (a spoke joins only two points). Points and the connecting lines are
supplied to the generic ``polygons`` renderer (the lines are cosmetic --
adjacency lives in code). Points are addressed by their grid coordinate ``"x,y"``
on a 0..6 layout.
"""
...
# 16 points: outer ring + inner ring (each an 8-cycle), wired by four spokes at
# the mid-side points. Coordinates on a 0..6 grid (x right, y down).
...
def _ring_adj():
    ...
    for ring in RINGS:
        ...
        for i in range(8):
            ...
            adj[ids[i]].add(ids[(i - 1) % 8])
    # spokes connect the mid-side point of the outer ring to that of the inner
    # ring (indices 1,3,5,7 in each 8-cycle).
    for k in (1, 3, 5, 7):
...
def _mills():
    # Eight mills: four edges of three points on each of the two squares. There
    # are NO mills along the spokes (a spoke joins only two points).
    mills = []
    for ring in RINGS:
        ids = [f"{x},{y}" for (x, y) in ring]
        for k in (0, 2, 4, 6):
...
def _line_segments():
    ...
    for k in (1, 3, 5, 7):
        segs.append([list(OUTER[k]), list(INNER[k])])
    return segs
...
class SixMensMorris(Game):
    ...
    def legal_moves(self, state):
        ...
        # movement phase -- always to an adjacent empty point (no flying)
        out = []
