"""Achi -- a traditional Ghanaian alignment game on a 3x3 grid of 9 points.

Achi is the close cousin of Three Men's Morris / Tapatan, distinguished by the
piece count: each player has **four** pieces (not three). A two-phase game with
NO capture:

* PHASE 1 (placement): players alternate placing one piece on any empty point
  until all EIGHT pieces are on the board (four each). Because the board has nine
  points, exactly ONE point is left empty when placement ends.
* PHASE 2 (movement): a player slides one of their pieces along a board line into
  the (single) adjacent empty point.
...
A player WINS the instant their three pieces occupy one of the eight lines --
which can happen during placement or during movement. Following the standard
rule (Wikipedia, "Achi (game)"), a player who has no legal move LOSES (this is
reachable in Achi because, with eight pieces on nine points, the lone empty point
can end up with no friendly piece adjacent to it for the side to move). A
no-progress draw is declared after a hard cap of movement plies to guarantee
termination.
...
MEN = 4  # pieces per player -- this is what distinguishes Achi from Tapatan (3)
...
class AState:
    ...
    to_move: int = 0
    placed: list = field(default_factory=lambda: [0, 0])   # pieces placed per player
    move_plies: int = 0                                 # movement-phase plies so far
    winner: object = None                               # set when someone forms a line / is stuck
...
class Achi(Game):
    ...
    # game on a 9-point board with only one empty point.
    DRAW_MOVE_PLIES = 80
    ...
    def _slides(self, state, pl):
        """All slide moves available to `pl` in the movement phase."""
        out = []
    ...
    def legal_moves(self, state):
        ...
        # movement phase: slide a piece along a line to the (single) empty point
        return self._slides(state, pl)
    ...
    def apply_move(self, state, move, rng=None):
        ...
        ns = AState(pos=pos, to_move=1 - pl, placed=placed,
                    move_plies=move_plies, winner=winner)
        # Standard rule: a player who, on their turn, has no legal move loses.
        # Only meaningful once both sides have finished placing (during placement
        # there is always an empty point to drop on).
        if (ns.winner is None and not self._draw(ns)
                and self._both_placed(ns) and not self._slides(ns, ns.to_move)):
            ns.winner = pl                            # the side that just moved wins
        return ns
    ...
    def deserialize(self, d):
        return AState(pos=dict(d["pos"]), to_move=d["to_move"],
