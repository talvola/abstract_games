...
**Next-wave queue remaining (dedup-verified free):** wa_shogi, marseillais_chess, progressive_chess, monster_chess, lasker_morris, omweso, cheskers, picaria, latrunculi, nard, jul_gonu, yonin_shogi. NOTE: progressive/marseillais/monster chess need multi-move-per-turn handling (verify the engine supports it before assigning); cheskers/latrunculi/picaria have custom-render or clone-adjacency risk (dedup + browser-verify). AVOID clone-adjacent hunt/race variants.
