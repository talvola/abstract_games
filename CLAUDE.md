...
- **RenderSpec capabilities (opt-in, all generic):** off-board **reserve trays** + click-to-drop (`render` adds `reserve`; drop move `"L@c,r"` — Crazyhouse/Shogi); cosmetic **`board.lines`** (connecting-line segments in cell-coordinate space — alquerque/Morris); per-cell **`board.tints`** (`{cellId: colour}` terrain — Jungle river); **stacking** towers (a piece carries `stack: [owner,…]` bottom→top → drawn as layered owner-coloured bands + height badge — Lasca). `GAME_STATUS.md` (generated by `engine/tools/gen_game_status.py`) is the live per-game catalogue (board, verification anchor, test/browser status).
