...
--------------------------------------------------------------------------
Piece movement, check/mate, the draw rules, deserialisation and rendering come
from ``agp.hexchesslike`` (shared with the other five classical hex chesses --
the direction tables are identical in axial space for the whole family). This
module supplies the board, the army and the three rule deltas from McCooey's
full-size game.
...
* Pawn: one vacant cell straight forward; captures one cell along the two
  ...
  NO initial double step and therefore NO en passant — so this game's state
  has no ``ep`` field at all (see ``MState`` below) and its serialised form
  carries no ``ep`` key.
...
* Check/checkmate as in chess, and checkmate ENDS THE GAME AT ONCE — it
  outranks the 50-move / repetition counters (the shared ``_draw_reason``).
  STALEMATE IS A DRAW — McCooey's rule, explicitly not Gliński's 3/4-1/4.
...
# The names re-exported here (MState, ORTHO/DIAG/KNIGHT, CELLS, on_board,
# cell_name, _cell, _is_promo, _poskey, _in_check, PAWN_FWD/PAWN_CAPS,
# PROMO_CHOICES, PLY_CAP, WHITE/BLACK) are the module's PUBLIC SURFACE:
# `selftest.py` imports them, and the selftests are the regression net for this
# refactor, so they are deliberately not rewritten. Keep these names.
from agp.hexchesslike import (BLACK, DIAG, KNIGHT, ORTHO, WHITE,  # noqa: F401
                              HexChessLike, cell_str, parse_cell)
...
@dataclass
class _StateFields:
    """The fields of a Mini Hexchess position.

    Deliberately NOT ``agp.hexchesslike.HState``: that carries an ``ep`` field,
    and this game has no double step, so there is no en-passant state to carry.
    """
    ...
    castling: frozenset = frozenset()           # always empty: no castling


class MState(_StateFields):
    """A state with NO ``ep`` attribute at all.

    The shared core always passes ``ep=`` when it builds a state (every other
    member of the family has en passant). Here the keyword is accepted and
    DROPPED, so ``hasattr(state, "ep")`` is False and the absence of the rule
    is structural rather than a None that has to be remembered.
    """

    def __init__(self, *args, ep=None, **kw):
        super().__init__(*args, **kw)


class MiniHexchess(HexChessLike):
    CELLS = frozenset(CELLS)
    FILES = FILES
    NAME_OF = NAMES
    PLY_CAP = PLY_CAP
    PROMO_CHOICES = PROMO_CHOICES      # R/B/N -- never a queen
    STATE = MState
    # STALEMATE_SCORED stays False: stalemate is an ordinary DRAW, McCooey's
    # rule.  No castling and no en passant, so `castle_moves` / `ep_after`
    # keep the core's do-nothing defaults.
    ...
    def initial_state(self, options=None, rng=None):
        # Same as the core's, minus the `ep` this game does not have. The
        # opening position is SEEDED into `reps`: without it the start
        # position is only counted from its first recurrence, so a threefold
        # repetition of it would need four occurrences instead of three.
        s = self.STATE(board=self.setup_board(), to_move=WHITE)
        s.reps = {self.poskey(s.board, s.to_move, None, s.castling): 1}
        return s
    ...
    def serialize(self, s) -> dict:
        # The core's version writes `ep` and `castling`; this game has neither,
        # and the server stores this dict in the DB, so the on-disk shape is
        # kept exactly as it shipped: six fields, no more.
        return {
...
# `selftest.py` calls `_in_check` / `_poskey` as module-level functions (the
...
def _poskey(board: dict, to_move: int) -> str:
    return _G.poskey(board, to_move, None, frozenset())
