"""Nine Men's Morris -- the classic mill game on the 24-point board.

A two-phase game: first each player *places* nine men one at a time, then they
...
The board is the standard three concentric squares with the four mid-side spokes;
points and the connecting lines are supplied to the generic ``polygons`` renderer
(the lines are cosmetic -- adjacency lives in code). Points are addressed by their
